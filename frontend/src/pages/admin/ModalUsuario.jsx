        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity duration-300">
            ...
            >
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                    <div className="flex items-center">
                        <div className={`p-2 rounded-lg mr-3 ${
                            modalType === 'crear' ? 'bg-blue-100 text-blue-600' : 
                            modalType === 'password' ? 'bg-green-100 text-green-600' : 
                            'bg-purple-100 text-purple-600'
                        }`}>
                            {modalType === 'crear' && <User className="h-5 w-5" />}
                            {modalType === 'password' && <Key className="h-5 w-5" />}
                            {modalType === 'role' && <Shield className="h-5 w-5" />}
                        </div>
                        <h3 className="text-xl font-semibold text-gray-900">
                            {modalType === 'crear' && 'Crear Nuevo Administrador'}
                ...
                <div className="p-6 max-h-[70vh] overflow-y-auto">
                    {modalType === 'crear' && (
                        <form onSubmit={(e) => { e.preventDefault(); handleCreateUser(); }} className="space-y-5">
                            <div>
                            ...
                            <div className="flex space-x-4 pt-2">
                                ...
                                <button
                                    type="submit"
                                    className="cursor-pointer flex-1 px-4 py-3 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg font-medium"
                                >
                                    Crear Usuario
                                </button>
                            </div>
                    ...
                    {modalType === 'password' && selectedUser && (
                        <form onSubmit={(e) => { e.preventDefault(); closeModal(); }} className="space-y-5">
                            <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
                                <h4 className="font-medium text-blue-900">Usuario: {selectedUser.nombre}</h4>
                                <p className="text-sm text-blue-700 mt-1">DNI: {selectedUser.dni}</p>
                            </div>
                    ...
                    {modalType === 'role' && selectedUser && (
                        <div className="space-y-5">
                            <div className="p-4 bg-purple-50 rounded-xl border border-purple-100">
                                <h4 className="font-medium text-purple-900">Usuario: {selectedUser.nombre}</h4>
                                <p className="text-sm text-purple-700 mt-1">DNI: {selectedUser.dni}</p>
                                <p className="text-sm text-purple-700 mt-1">Rol actual: {selectedUser.rol}</p>
                            </div>
