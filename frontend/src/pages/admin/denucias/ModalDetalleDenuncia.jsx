...
export default function ModalDetalleDenuncia({
    ...
    const estadoColors = {
        recibida: "border-blue-300 text-blue-700 bg-blue-50",
        en_proceso: "border-amber-300 text-amber-700 bg-amber-50",
        aceptada: "border-emerald-300 text-emerald-700 bg-emerald-50",
        rechazada: "border-red-300 text-red-700 bg-red-50",
        finalizada: "border-slate-300 text-slate-700 bg-slate-50",
    };
	...
	const onUpdate = async () => {
		...
		if (data.comentario.trim() === "" || data.estado.trim() === "") {
			toast.error(
				"Por favor, coloca un comentario y selecciona un estado."
			);
			return;
		}
		setIsUpdating(true);
		try {
			const response = await axios.post(
				`${API_URL}/admin/procesos-denuncia`,
				data,
				{
			...
			if (axios.isAxiosError(err)) {
				toast.error(
					"Error al actualizar la denuncia. Inténtalo de nuevo."
				);
				console.error(
				...
				<div className="p-6 flex flex-col gap-6">
					...
					<div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
						...
						<div className="space-y-2">
							{Array.isArray(denuncia.historial) &&
							denuncia.historial.length > 0 ? (
								denuncia.historial.map((item, idx) => {
									const colorClass =
										estadoColors[item.estado] ||
										"border-slate-400 text-slate-700 bg-slate-50";
									const textClass =
										(colorClass.split(" ") || []).find(
											(c) => c.startsWith("text-")
										) || "text-slate-700";
									return (
										<div
											key={idx}
											className={`bg-white p-3 rounded-lg border-l-4 shadow-sm ${colorClass}`}
										>
											...
											<div
												className={`font-bold mb-1 ${textClass}`}
											>
												{item.estado.split("_").join(" ").toUpperCase()}
											</div>
								...
								<div className="text-sm text-slate-500">
									No hay historial para esta denuncia.
								</div>
							)}
						</div>
					</div>
