...
export default function ModalAgregarDenunciado({
	open,
	onClose,
	callback,
	closeModal,
	...
	useEffect(() => {
		const fetchDniData = async () => {
			if (tipoDocumento !== "DNI" || form.dni.trim().length !== 8) return;
			setIsFetching((prev) => ({
				...prev,
				dni: true,
			}));
			try {
				const data = await getServiceData("dni", form.dni);
				if (data) {
					setForm((prev) => ({
						...prev,
						nombres: data.nombre,
		...
		const getRucData = async () => {
			if (tipoDocumento !== "RUC" || form.ruc.trim().length !== 11)
				return;
			setIsFetching((prev) => ({
				...prev,
				ruc: true,
			...
			try {
				const data = await getServiceData("ruc", form.ruc);
				if (data) setForm((prev) => ({ ...prev, razonSocial: data }));
			} finally {
				setIsFetching((prev) => ({
					...prev,
					ruc: false,
		...
		if (tipoDocumento === "DNI" && form.dni.trim().length === 8) {
			fetchDniData();
		} else if (tipoDocumento === "RUC" && form.ruc.trim().length === 11) {
			getRucData();
		}
	}, [tipoDocumento, form.dni, form.ruc]);
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await axios.post(
				`${API_URL}/admin/add-denunciado/${denunciaId}`,
				{
                    tipo_documento: tipoDocumento,
                    nombre: form.nombres || null,
                    documento: form.dni || form.ruc || null,
                    razon_social: form.razonSocial || null,
                    representante_legal: form.representante || null,
			...
			if (response.data.success || response.status === 200) {
				...
				toast.error(
					"Error al agregar el denunciado. Inténtelo de nuevo."
				);
			}
		} catch (error) {
				...
				<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div className="grid grid-cols-3 gap-3">
							{documentOptions.map((option) => (
								<motion.button
									...
									whileTap="tap"
									onClick={() =>
										handleTipoDocumento(option.value)
									}
									className={`
					...
					<div className="flex justify-end gap-3 pt-4">
						...
						<button
							...
							className="cursor-pointer px-5 py-2.5 rounded-lg font-medium bg-blue-600 text-white hover:bg-blue-700 transition disabled:cursor-not-allowed disabled:bg-blue-300"
						>
							{isSubmitting ? <Loader size={6} /> : "Guardar"}
						</button>
					</div>
				</form>
			</motion.div>
